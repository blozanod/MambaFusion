...
| `synthetic_burst_generation.py` | `data/synthetic_burst_generation.py` | 2 import lines re-pointed to this package (`data.camera_pipeline` → `burstISP.data.dbsr.camera_pipeline`, `utils.data_format_utils` → `burstISP.data.dbsr.data_format_utils`); added `indexing='ij'` to the one `torch.meshgrid()` call in `single2lrburst` (silences the deprecation warning PyTorch now issues when it's omitted — `'ij'` is the exact behavior the call already had, so no numeric change; this call runs on every training sample, so the warning fired continuously during training). Everything else verbatim |
